...
As the company logo is red, I chose to use a red #9d080b from the logo as the main color for creating backgrounds, hover effects and hr elements etc and a white background for the entire site to help the red jump of the page.

I used #f8f8f8 for the background of the contact form as this worked well with the red and white color scheme already in place, and helped to break this page up slightly. It also helped it to stand out from the other elements, as the contact form is the most important part of the page.
...
- The user is given links to each page on the website, where the page currently being viewed is made clear with the darker text.
- Each option is clearly labelled & easy to read.
- On smaller screens the page links are reduced to a hamburger menu to optimize the user's experience.
...
- Links to social media were also included as this is such a huge part of company marketing these days. Customers are encouraged to follow the company and keep up to date with their latest updates via social media.
- Each feature within the footer is styled appropriately to match the company's branding.
...
- I chose to use this image as it's not only powerful but also gives a clear indication of what the company is offering. This is the first thing a user sees when they enter the website, so it grabs the users attention instantly and gives them a good idea of what's to come.
...
- The company's customer reviews were included on the home page so that users could see what previous customers of the company think about their products and quality of service.
...
- This feature is neatly broken up into three rows, one for each product, with the company's red branding shown in a reverse grid pattern.
...
- The image was also chosen as the staff are wearing kilts themselves, in a tartan where the colors match with the red theme of the website and the branding.

### About Us - Our Story:

![About Us Our Story](documentation/screenshots/aboutusourstoryscreenshot.png)

- This section gives some information on the company's history and stresses how much they've grown over the years as a business.
...
- One of the site aims was to promote this service that the company offers, so I decided to use the strong red background with white text, to really make it stand out. I also included a home icon using Fontawesome, to draw the user's eye to this feature.

### Contact - Contact Form:

![Contact Form](documentation/screenshots/contactformscreenshot.png)

- This feature allows the user to submit their details along with a message to the company, so they can respond in their own time.
...
- Once submitted, the page redirects to a thank you page where the user is told the company will reply ASAP (this feature can be seen below).
...
- The Google Map feature allows the user to see where the company's store is actually located on a physical map, which is much easier and more relatable than simply looking at a physical address.
...
- A 'Why use us?' page could be added showing the company's USP's (unique selling points), with images to support each USP. For example, kilts are actually made on-site, so a photo of someone making a kilt backed up by an image from behind the scenes.
...
- Every page link directs the user to the relevant page, from anywhere in the website. When viewing a page, the active page should be highlighted with darker text in the navigation bar. This is the case for all four of the main pages. When the user reaches the thank-you.html or 404.html pages, no 'active' page is visible on the navigation bar although the user can then go to any of the four pages that are included in the navigation from there.
...
- Also on the contact page, the email link was tested to ensure when it's clicked or tapped, a new email is started on the user's device to the correct email address, to avoid manual input error if the user decided to write this down and type the email themselves. When the email was clicked, a new email was started with the email address already filled in.
- A 404.html page was added for instances where the website can't find the page the user is trying to access. Although a default 404 error page would show automatically, I didn't want the customer to be directed off the website and give them the option to go to another page via the navigation bar, instead of a white page with a simple error message. To test if the customer is redirected to 404.html automatically where a 404 error occurs, I changed the /page section of the URL to various random number and letter sequences and in all cases the default 404.html page was shown.
...
**Achieved?**: Yes. Each page clearly shows the subject matter through clear images and concise text.

**User story 2**: As a user, I want to navigate the website to find what I require quickly and easily.

**Achieved?**: Yes. Every page is quickly and easily accessible via the navigation bar which features throughout the website. Each page is a good length, long enough to fit the content required but not too long causing the user to get lost and lose interest.

**User story 3**: As a user, I want to know which tartans are available to hire

**Achieved?**: Yes. The kilt hire page displays the three tartans available to hire along with some information on each tartan and the prices to hire them.

**User story 4**: As a user, I want to find out how much it is to hire a kilt

**Achieved?**: Yes. The kilt hire page includes prices for each hire outfit available.
...
- Contact Form
  ...
  - Problem: When submitted, the contact form was not redirecting the user to the thank-you.html page.
...
- Accessibility error when testing with Lighthouse
  ...
  - Cause: The aria-description attribute used within these pages
